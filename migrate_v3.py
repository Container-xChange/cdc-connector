...
MAX_WORKERS = int(os.environ.get('MIGRATION_WORKERS', '8'))  # Parallel tables (reduced for memory)
PREFETCH_SIZE = int(os.environ.get('MIGRATION_PREFETCH', '200000'))  # MySQL fetch size
LARGE_TABLE_THRESHOLD = int(os.environ.get('LARGE_TABLE_THRESHOLD', '1000000'))  # 1M rows
CHUNK_WORKERS = 10  # Fixed workers for large table ID-based chunking
CHUNK_SIZE = 1000000  # Rows per chunk for ID-based chunking
...
def create_table(pg_conn, schema_name, table_name, columns, skip_constraints=False, unlogged=False, table_prefix=''):
    """Create table in Postgres without indexes/constraints for fast loading (does NOT drop existing table)"""
    table_lower = (table_prefix + table_name).lower()
...
def create_indexes_and_constraints(pg_conn, schema_name, table_name, primary_keys, indexes, foreign_keys, table_prefix=''):
...
def find_primary_key(mysql_conn, table_name):
    """Find the primary key column (preferably a single numeric column for chunking)"""
    with mysql_conn.cursor() as cur:
        cur.execute(f"DESCRIBE {table_name}")
        for row in cur.fetchall():
            col_name, col_type, nullable, key, default, extra = row
            ...
            # Look for PRIMARY KEY that's numeric (int, bigint, etc.)
            if key == 'PRI' and any(t in col_type_lower for t in ['int', 'bigint', 'smallint', 'mediumint']):
                return col_name

        return None

def get_id_ranges(mysql_conn, table_name, id_column, chunk_size):
    """Get ID ranges for chunking large tables"""
    with mysql_conn.cursor() as cur:
        # Get min/max IDs
        query = f"""
            SELECT
...
def migrate_id_chunk(mysql_config, pg_schema, table_name, id_column, id_range, columns, table_prefix=''):
    """Migrate a single ID range chunk of data"""
    mysql_conn = get_mysql_conn(mysql_config)
    pg_conn = get_pg_conn()
    ...
    try:
        ...
        # Count rows for this ID range
        with mysql_conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {id_column} BETWEEN {start_id} AND {end_id}")
        ...
        try:
            # Fetch data for this ID range in batches
            ...
            while offset < total:
                ...
                if batch_count > 0:
                    try:
                        ...
                        # Show progress every 10% within this ID chunk
                        progress = 100 * processed // total if total > 0 else 100
                        if progress >= last_progress + 10:
                            elapsed = (datetime.now() - start_time).total_seconds()
                            rate = processed / elapsed if elapsed > 0 else 0
                            print(f"      ID {start_id:,}-{end_id:,}: {processed:,}/{total:,} ({progress}%) - {rate:.0f} rows/sec")
                            last_progress = progress
                    except Exception as e:
                        print(f"      ID {start_id:,}-{end_id:,}: Error copying batch at offset {offset}: {e}")
                        pg_conn.rollback()
                    finally:
                        csv_buffer.close()
            ...
            # Commit all batches for this ID range
            pg_conn.commit()
        ...
        elapsed = (datetime.now() - start_time).total_seconds()
        return id_range, processed, elapsed

    except Exception as e:
        print(f"      ID {start_id:,}-{end_id:,}: ERROR: {e}")
        return id_range, 0, 0
    finally:
        mysql_conn.close()
        pg_conn.close()

def migrate_table_data(mysql_config, pg_schema, table_name, table_prefix=''):
    """Migrate data for a single table - uses parallel ID-based chunking for large tables"""
    mysql_conn = get_mysql_conn(mysql_config)
    pg_conn = get_pg_conn()
    ...
    try:
        ...
        # FOR LARGE TABLES: Use parallel ID-based chunking
        if total >= LARGE_TABLE_THRESHOLD:
            id_column = find_primary_key(mysql_conn, table_name)

            if id_column:
                print(f"    {table_name}: Large table ({total:,} rows) - using parallel ID-based chunking on '{id_column}'")
                id_ranges = get_id_ranges(mysql_conn, table_name, id_column, CHUNK_SIZE)

                if id_ranges and len(id_ranges) > 1:
                    # Fixed workers for large table chunking
                    print(f"    {table_name}: Chunking into {len(id_ranges)} ranges with {CHUNK_WORKERS} parallel workers")

                    # Close connections - each worker will create their own
                    mysql_conn.close()
                    pg_conn.close()

                    # Migrate ID ranges in parallel
                    with ThreadPoolExecutor(max_workers=CHUNK_WORKERS) as executor:
                        futures = {
                            executor.submit(migrate_id_chunk, mysql_config, pg_schema, table_name, id_column, id_range, columns, table_prefix): id_range
                        ...
                        for future in as_completed(futures):
                            id_range = futures[future]
                            range_result, rows_processed, elapsed = future.result()
                            total_processed += rows_processed
                            completed_chunks += 1

                            if rows_processed > 0:
                                rate = rows_processed / elapsed if elapsed > 0 else 0
                                progress_pct = 100 * completed_chunks // len(id_ranges)
                                print(f"      ID {id_range[0]:,}-{id_range[1]:,}: ✅ {rows_processed:,} rows in {elapsed:.1f}s ({rate:.0f} rows/sec) | Overall: {completed_chunks}/{len(id_ranges)} chunks ({progress_pct}%)")
                    ...
                    print(f"    {table_name}: Only 1 chunk needed, falling back to sequential migration")
            else:
                print(f"    {table_name}: No numeric primary key found, falling back to sequential migration")

        # FALLBACK: Sequential migration for small tables or tables without numeric primary key
...
def migrate_table(mysql_config, pg_schema, table_name, database_name, table_prefix=''):
    ...
    try:
        ...
        success = migrate_table_data(mysql_config, pg_schema, table_name, table_prefix=table_prefix)
        ...
        pg_conn = get_pg_conn()
        try:
            create_indexes_and_constraints(pg_conn, pg_schema, table_name, primary_keys, indexes, foreign_keys, table_prefix=table_prefix)
...
def migrate_database(db_name, config, table_filter=None, max_workers=None):
    ...
    # Get database name for FK extraction and table prefix
    database_name = get_env_value(config['mysql']['database'])
    table_prefix = config.get('prefix', '')
    ...
    print(f"  Table prefix: {table_prefix if table_prefix else '(none)'}")
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(migrate_table, config['mysql'], config['schema'], table, database_name, table_prefix): table
            for table in tables
        }
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Migrate all tables from trading database (from .env allowlist)
  python3 migrate_v3.py --database trading --tables all

  # Migrate specific tables from trading database with 5 workers
  python3 migrate_v3.py --database trading --tables T_ABSTRACT_OFFER,T_DEAL --max-workers 5

  # Migrate single table from finance database with default workers
  python3 migrate_v3.py --database finance --tables T_INVOICE

  # Migrate with custom batch size and threshold (ID-based chunking with 10 workers)
  python3 migrate_v3.py --database trading --tables T_DEAL --batch-size 50000 --threshold 500000
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Comma-separated list of tables to migrate (e.g., T_DEAL,T_CARRIER) or "all" to migrate all tables from the database allowlist in .env'
    )
...
def main():
    ...
    print(f"  ✅ Parallel ID-based chunking for large tables ({CHUNK_WORKERS} workers)")
    print(f"  ✅ Chunk size: {CHUNK_SIZE:,} rows per ID range")
    ...
    if args.tables.lower() == 'all':
        # Get tables from allowlist in .env
        allowlist_env_key = f'{args.database.upper()}_TABLE_ALLOWLIST'
        allowlist_str = os.environ.get(allowlist_env_key, '')
        if not allowlist_str:
            print(f"\n❌ Environment variable {allowlist_env_key} not found or empty")
            print(f"   Set {allowlist_env_key} in .env file with comma-separated table list")
            sys.exit(1)

        # Parse allowlist - format is "database.table1,database.table2"
        ...
        for item in allowlist_str.split(','):
            ...
            if '.' in item:
                ...
                table_filter.append(table_name)
            else:
                table_filter.append(item)

        print(f"📋 Using tables from {allowlist_env_key}:")
        print(f"   {', '.join(table_filter)}")
    else:
